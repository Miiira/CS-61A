...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    rolled_times, sum, pig_out = 0, 0, 0
    while rolled_times < num_rolls:
        rolling_outcome = dice()
        if rolling_outcome == 1:
            pig_out = True
        sum += rolling_outcome
        rolled_times += 1
    if pig_out:
...
def both(f, g):
    ...
    def new_comment(score0, score1):
        returned_f = f(score0, score1)
        returned_g = g(score0, score1)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def highest(score0, score1):
        def checker(score):
            ...
            high = previous_high
            if score - previous > high:
                high = score - previous
                print(high, is_one_point(high), "That's the biggest gain yet for Player", who)
            return high
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, best_num, max_score = 1, 1, 0
    while num_rolls <= 10:
        score = make_averaged(roll_dice, num_samples)(num_rolls, dice)
        if score > max_score:
            max_score, best_num = score, num_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if not bacon_strategy(score, opponent_score, margin, num_rolls):
        return 0
    elif is_swap(score + result, opponent_score):
        if opponent_score > score + result:
            return 0
    return num_rolls
    # END PROBLEM 11
