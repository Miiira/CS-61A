...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        new_env = self.make_call_frame(operands, env)
        eval_operands = operands.map(lambda operand: Thunk(operand, new_env))
        result = complete_eval(eval_all(self.body, new_env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, 
            do_lambda_form(Pair(target.second, expressions.second), env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        macroform = MacroProcedure(target.second, expressions.second, env)
        env.define(target.first, macroform)
        return target.first
    else:
        raise SchemeError
    # END Problem 21
